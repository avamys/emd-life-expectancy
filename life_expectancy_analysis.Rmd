...
First let's see the size and print couple of records to see the structure of the data set
```{r dim}
dim(df)
```
...
As we can see the dataset contains `r nrow(df)` records of `r ncol(df)` features. Zawiera on informacje o ogólnym stanie gospodarki oraz o badaniach medycznych dla danego kraju w danym roku.
...
Kolejny atrybut jaki postanowiono sprawdzic 

```{r missing_thin, cache=TRUE}
thinness <- df %>%
...
# CZY COUNTRY REZ????
df_clean <- select(df_clean, -c(Status, Country, `infant deaths`, 
                                `percentage expenditure`, `thinness 5-9 years`))
```

Podzielono dane na zbiory train, test oraz validate
```{r split}
spec = c(train = .8, test = .2)

g = sample(cut(
  seq(nrow(df_clean)), 
  nrow(df_clean)*cumsum(c(0,spec)),
  labels = names(spec)
))

res = split(df_clean, g)
```

```{r standarization}
res$train <- as.data.frame(scale(res$train))
res$test <- as.data.frame(scale(res$test))
```

```{r regression}
linearModel = lm(`Life expectancy` ~ ., data=res$train)
summary(linearModel)
```

```{r prediction}
pred1 <- predict(linearModel, newdata = res$test)
rmse <- sqrt(sum((exp(pred1) - res$test$`Life expectancy`)^2)/length(res$test$`Life expectancy`))
